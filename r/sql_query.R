...
# Can change if mortality data for 16/17 available.
years <- create_year_vector(0506, 10)
...
for(i in seq_along(years)){
  ...
  year_plus_one <- ifelse(str_length(as.integer(str_sub(years[i], -2))+1) <2, 
                          str_c("0", as.integer(str_sub(years[i], -2))+1),
                          str_c(as.integer(str_sub(years[i], -2))+1) # makes dbl a chr
                          )
...
# saveRDS(ip_data_2, here("data", "ip_data.RDS"))
# getwd()
